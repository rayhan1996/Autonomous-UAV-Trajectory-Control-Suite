...
async def safety_watchdog(
    ...
    *,
    t0: float,  # ✅ shared mission start time reference (same as fly_circle)
    ...
    """
    Safety watchdog that monitors:
      - mission timeout
      - XY drift from reference trajectory
      - speed magnitude
      - roll/pitch limits

    IMPORTANT:
      Uses the SAME time reference as the mission (t = time.time() - t0).
    """
    ...
    while state.running and not state.emergency_stop:
        ...
        if (
            state.pos_ned is None
            or state.vel_ned is None
            or state.attitude_deg is None
        ):
            continue

        # ✅ Shared time reference with mission
        t = time.time() - t0
        ...
        if abs(roll) > roll_pitch_max_deg or abs(pitch) > roll_pitch_max_deg:
            ...
            state.emergency_reason = (
                f"UNSAFE ATTITUDE: roll={roll:.1f}, pitch={pitch:.1f}"
            )
            break
